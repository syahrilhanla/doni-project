...
const Dashboard = () => {
  ...
  useEffect(() => {
    if (user.title) {
      ...
      if (
        user.title[0].isApprovedByProfOne === "Denied" ||
        user.title[0].isApprovedByProfTwo === "Denied"
      ) {
        setDeniedTitle(true);
      }
    }
  ...
  const handleNewTitle = async () => {
    try {
      ...
      const profRef1 = query(
        collection(db, "professorList"),
        where("name", "==", user.profOne)
      );
      const prof1Data = await (await getDocs(profRef1)).docs[0].id;
      const profRef2 = query(
        collection(db, "professorList"),
        where("name", "==", user.profTwo)
      );
  ...
  return (
    <Layout>
      ...
      <div className="h-full px-4  py-4">
        <div className="flex lg:space-between xxs:max-sm:flex-col xxs:flex-col sm:flex-col flex-row md:flex-col sm:max-md:flex-col md:max-lg:flex-col  mt-5 mb-2 mx-4">
          <div className="flex lg:flex-row xxs:flex-col  sm:flex-col">
            ...
            <div className="flex  justify-center xxs:w-full items-center sm:max-md:w-full  md:max-lg:w-full  md:max-lg:mt-3 w-3/5 h-24 bg-[#f2e8f24f] text-[#683ab7d5] rounded-lg shadow-md">
              {judul ? (
                    ...
                    <div className="w-full flex justify-center">
                      ...
                      {!deniedTitle && !terimaProfSatu && !terimaProfDua && (
                        <div className="w-full">
                          <p>{judul}</p>
                          <p>
                            {"(Menunggu Acc Dosen Pembimbing Satu dan Dua)"}
                          </p>
                        </div>
                      )}
                      {!deniedTitle && terimaProfSatu && !terimaProfDua && (
                      ...
                      {deniedTitle && (
                        <div className="w-full flex flex-col sm:flex-row sm:gap-2 gap-3  justify-between">
                          <div className="w-full flex flex-col">
                            <p>{judul}</p>
                            <p className="text-red-400 font-semibold">
                              Judul ditolak oleh salah satu atau kedua dosen.
                              Ajukan kembali.
                            </p>
                          </div>
                          <div className="px-4">
                ...
                <>
                  <div className="flex flex-col sm:flex-row sm:gap-2 gap-3 justify-between w-full mx-7">
        ...
        <div className="flex justify-center xxs:max-sm:flex-col sm:max-md:flex-col md:max-lg:flex-col mt-7 mx-3">
          ...
          <div
            className={`grid justify-center xxs:max-sm:w-full sm:max-md:w-full md:max-lg:w-full mr-2 py-5 px-4 w-1/3 h-40 bg-[#f1e8f252] border-4 ${
              seminar ? "border-4 border-[#caf3e0]" : "border-[#f3caca]"
            } text-[#707070] rounded-2xl shadow-xl`}
          >
            <div className=" text-xl flex justify-center items-center ">
